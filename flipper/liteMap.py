...
class liteMap:
    ...
    def loadDataFromHealpixMap(self, hpm, interpolate = False, hpCoords = "J2000"):
        ...
        phOut *= numpy.pi/180
        thOut = 135. - thOut #polar angle is 0 at north pole
        thOut *= numpy.pi/180
...
def liteMapFromFits(file,extension=0):
    ...
    trace.issue("flipper.liteMap",1, "Map dimensions (Ny,Nx) %d %d"%\
                (ltmap.Ny,ltmap.Nx))
    trace.issue("flipper.liteMap",1, "pixel scales Y, X (degrees) %f %f"%\
